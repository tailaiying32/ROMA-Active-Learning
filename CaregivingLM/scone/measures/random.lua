...
function init( model, par, side )
	...
	rand_x = tonumber(scone.target_x)
	rand_y = tonumber(scone.target_y)
	rand_z = tonumber(scone.target_z)
	
	targets = {
		vec3:new(rand_x, rand_y, rand_z),
	}
	
	-- Store target position for logging
	target_x = rand_x
	target_y = rand_y  
	target_z = rand_z
	cur_trg_idx = 0
...
function store_data( frame )
	...
	frame:set_value( "reach.target.x", target_x )
	frame:set_value( "reach.target.y", target_y )
	frame:set_value( "reach.target.z", target_z )
end
